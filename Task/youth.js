...
if(!$.isNode()&&cookieYouth.indexOf("#") ==-1){
 cookieArr.push($.getdata('youthheader_zq'));
    ...
    if (process.env.WITHDRAW_URL && process.env.WITHDRAW_URL.indexOf('#') > -1) {
        withdrawUrl = process.env.WITHDRAW_URL.split('#');
    } else if (process.env.WITHDRAW_URL && process.env.WITHDRAW_URL.indexOf('\n') > -1) {
        withdrawUrl = process.env.WITHDRAW_URL.split('\n');
    } else {
        withdrawUrl = [process.env.WITHDRAW_URL]
    };
    if (process.env.WITHDRAW_BODY && process.env.WITHDRAW_BODY.indexOf('#') > -1) {
        withdrawBody = process.env.WITHDRAW_BODY.split('#');
    } else if (process.env.WITHDRAW_BODY && process.env.WITHDRAW_BODY.indexOf('\n') > -1) {
        withdrawBody = process.env.WITHDRAW_BODY.split('\n');
    } else {
